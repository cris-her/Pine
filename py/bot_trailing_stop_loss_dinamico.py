...
def diff(list1, list2):
    r = list(itertools.filterfalse(lambda x: x in list1, list2)) + \
        list(itertools.filterfalse(lambda x: x in list2, list1))
    return r


def _dinamic_stop_loss_(symbol_to_buy, quantity_to_buy, price_to_buy):
    prevPrice_of_Symbol = float(price_to_buy)
    prevPrice_to_Take = truncate(
        float(price_to_buy) - float(price_to_buy) * 0.03, 4)
    prevStopPrevPrice_to_Take = truncate(
        float(prevPrice_to_Take) + float(prevPrice_to_Take) * 0.01, 4)
    try:
        ...
        while 1:
            ...
            # DEBO ENCONTRAR UNA FUNCION QUE ME DE EL PRECIO ACTUAL DE UN TICKET
            currentPrice_to_Take = truncate(
                float(currentPrice_of_Symbol) - float(currentPrice_of_Symbol) * 0.03, 4)
            currentStopCurrentPrice_to_Take = truncate(
                float(currentPrice_to_Take) + float(currentPrice_to_Take) * 0.01, 4)
            if float(prevStopPrevPrice_to_Take) < float(currentStopCurrentPrice_to_Take):
                ...
                prevStopPrevPrice_to_Take = float(
                    currentStopCurrentPrice_to_Take)
        ...
        with open(symbol_to_buy+".txt", "a") as myfile:
            myfile.write(str(datetime.datetime.now()) + " - an exception occured - {}".format(
                e) + " Oops! ERORR EN EL DINAMIC LOSS   occurred.\n")
    ...
    try:
        ...
        with open("test.txt", "a") as myfile:
            myfile.write(str(datetime.datetime.now()) +
                         " - an exception occured - {}".format(e) + " WHILE CRASH  \n")
        client = Client(config.API_KEY, config.API_SECRET, tld='com')
...
for i in range(newCryptos, len(currentCryptos)):
    try:
        ...
        if amount == 0:
            continue
        if amount > 0:
            quantity_to_buy = truncate(
                (float(amount) / float(price_to_buy)), 3)

            with open("test.txt", "a") as myfile:
                myfile.write(str(datetime.datetime.now()) + " - " + symbol_to_buy +
                             " ; quantity:" + str(quantity_to_buy) + " ; Price " + str(price_to_buy) + "\n")
            ...
            x = threading.Thread(target=_dinamic_stop_loss_, args=(
                symbol_to_buy, quantity_to_buy, price_to_buy))
            x.start()
            threadList.append(x)
    except Exception as e:
        ...
        with open("test.txt", "a") as myfile:
            myfile.write(str(datetime.datetime.now()) + " - an exception occured - {}".format(
                e) + " Oops !  IN FOR STATEMENT 178 occurred.\n")
        client = Client(config.API_KEY, config.API_SECRET, tld='com')
